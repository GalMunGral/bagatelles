...
export async function getStreamInfo(manifestUrl: string): Promise<StreamInfo> {
  return parseStreamInfo(await (await fetch(manifestUrl)).text());
}

function parseStreamInfo(xml: string): StreamInfo {
  ...
  const segDuration =
    Number(segTmplNode?.getAttribute("duration")) /
